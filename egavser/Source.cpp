#ifdef OLD_CODE_FOR_EG_ANTI_VIRUS_SERVICE
// easy lock folder service
#include <Windows.h>
#include <Wtsapi32.h>
#include <tchar.h>
#include "common.h"
...
{
	//OutputDebugString(_T("WinSEGAV AutoConfig: Main: Entry"));
	...
	{
		//OutputDebugString(_T("WinSEGAV AutoConfig: Main: StartServiceCtrlDispatcher returned error"));
		return  GetLastError();
	}

	//OutputDebugString(_T("WinSEGAV AutoConfig: Main: Exit"));
	return 0;
}
...
VOID WINAPI ServiceMain(DWORD argc, LPTSTR *argv)
{
	...
	//OutputDebugString(_T("WinSEGAV AutoConfig: ServiceMain: Entry"));

	g_StatusHandle = RegisterServiceCtrlHandler(SERVICE_NAME, ServiceCtrlHandler);

	if (g_StatusHandle == NULL)
	{
		//OutputDebugString(_T("WinSEGAV AutoConfig: ServiceMain: RegisterServiceCtrlHandler returned error"));
		goto EXIT;
	}
	...
	{
		//OutputDebugString(_T("WinSEGAV AutoConfig: ServiceMain: SetServiceStatus returned error"));
	}

	/*
	* Perform tasks neccesary to start the service here
	*/
	//OutputDebugString(_T("WinSEGAV AutoConfig: ServiceMain: Performing Service Start Operations"));

	// Create stop event to wait on later.
	g_ServiceStopEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
	if (g_ServiceStopEvent == NULL)
	{
		//OutputDebugString(_T("WinSEGAV AutoConfig: ServiceMain: CreateEvent(g_ServiceStopEvent) returned error"));
...
EXIT:
	//OutputDebugString(_T("WinSEGAV AutoConfig: ServiceMain: Exit"));
...
{
	//OutputDebugString(_T("WinSEGAV AutoConfig: ServiceCtrlHandler: Entry"));

	switch (CtrlCode)
	{
	case SERVICE_CONTROL_STOP:

		//OutputDebugString(_T("WinSEGAV AutoConfig: ServiceCtrlHandler: SERVICE_CONTROL_STOP Request"));
		...
		*/
		...
		if (SetServiceStatus(g_StatusHandle, &g_ServiceStatus) == FALSE)
		{
			//OutputDebugString(_T("WinSEGAV AutoConfig: ServiceCtrlHandler: SetServiceStatus returned error"));
		}
	...
	//OutputDebugString(_T("WinSEGAV AutoConfig: ServiceCtrlHandler: Exit"));
}
...
DWORD WINAPI ServiceWorkerThread(LPVOID lpParam)
{
	bool bEGAVSysRunning, bEGAVTrayRunning;
	...
	prgPath = prgPath.append(L"\\EGSoftWeb\\EG Anti Virus");
	std::wstring SysExe = prgPath;
	SysExe = SysExe.append(L"\\egavsys.exe");
	std::wstring TrayExe = prgPath + L"\\egavtray.exe";
	...
	while (WaitForSingleObject(g_ServiceStopEvent, 0) != WAIT_OBJECT_0)
	{
		...
		*/
		...
		bEGAVTrayRunning = IsApplicationRunning(L"egavtray.exe", exepid);
		...
		if (!bEGAVSysRunning)
		{
			...
			{
				ErrorServiceLogW(L"Error:CreateProcess:%d", GetLastError());
				return false;
			}
...
DWORD WINAPI ServiceWorkerThread(LPVOID lpParam)
{
	...
	std::wstring prgPath;
	GetProgramFilesFolderPath(prgPath);
	prgPath.append(L"\\EGSoftWeb\\EG Anti Virus");
	//std::wstring SysExe = prgPath;
...
BOOL InstallService()
{
	...
	schService = CreateService(schSCManager, SERVICE_NAME_EG_ANTI_VIRUS,
		L"Windows Service For EG Free AntiVirus", // service name to display
