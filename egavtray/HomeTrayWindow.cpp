...
/*
static const char * const title = "EG Anti Virus";
static const char * const message
= "EG Anti Virus is Active"
"\n"
"© 2016 EGSoftWeb";
*/
...
const wxString BaloonStrU(wxT("EG AntiVirus is up-to-date\n"));
const wxString BaloonStrN(wxT("EG AntiVirus is out-of-date\n"));
...
HomeTrayWin::HomeTrayWin(const wxString& title) : wxFrame(NULL, -1, title, wxDefaultPosition, wxSize(600, 450), EGAV_HOME_WINDOW_BORDER_STYLE)
{
	...
	m_update->Append(ID_TRAY_MENU_UPDATE_SOFTWARE, wxT("&Download Software Updates"));
...
{
	wxLaunchDefaultBrowser(wxT("http://www.egsoftweb.in/ContactUs.aspx"), wxBROWSER_NOBUSYCURSOR);
}
...
static void InitAboutInfoMinimal(wxAboutDialogInfo& info)
{
	...
	info.SetWebSite(wxT("www.egsoftweb.in"));
	info.SetIcon(AppIcon);
	info.SetName(EGAV_TITLE_NAME);
	info.SetVersion(wxT("2020"));
	info.SetCopyright(wxT("(C) 2018 EGSoftWeb, All Rights Reserved"));
}
...
void MyTaskBarIcon::OnMenuDownloadUpdates(wxCommandEvent&)
{
	...
	else
	{
		...
		//if no
		//download software and update  virus defination sig
	}
}
...
void HomeTrayWin::AutoUpdateVSD()
{
	...
	bool bIsVSDFileUpdated = ((vsdolddays >= 0) && (vsdolddays <= 2));
