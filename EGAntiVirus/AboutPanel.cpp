...
AboutPanel::AboutPanel(const wxString& title, wxWindow* parent) :wxPanel(parent, wxID_ANY, wxDefaultPosition, wxSize(550, 300))
{
	...
	m_st_heading = new wxStaticText(m_pHeadingText, -1, wxT("EG Free AntiVirus 2020"));
...

	{
		...
		info[1] = wxT(" © 2018 EGSoftWeb, All Rights Reserved\n");
		info[2] = wxT("Website:");
		info[3] = wxT(" http://www.egsoftweb.in \n");
		info[4] = wxT("Email:");
		info[5] = wxT(" < egsoftweb@gmail.com > \n");
		...
		m_tc_Copyright->SetStyle(infolenght[0], infolenght[1], *wxYELLOW);
		m_tc_Copyright->SetStyle(infolenght[1], infolenght[2], *wxGREEN);
		m_tc_Copyright->SetStyle(infolenght[2], infolenght[3], *wxWHITE);
		...
		m_tc_Copyright->SetStyle(infolenght[6], infolenght[7], *wxYELLOW);
	...
	m_bb_egsoftweb = new wxBitmapButton(m_pUpperBitmap, ID_BITMAP_BUTTON_EGAV, egavIcon, wxDefaultPosition, wxDefaultSize,EGAV_BUTTON_STYLE_NOBORDER);
	m_bb_egsoftweb->SetCursor(wxCursor(wxCURSOR_HAND));
	m_bb_egsoftweb->SetBackgroundColour(EGAV_PANEL_BACK_COLOR);

	m_sH_UpperBitmap = new wxBoxSizer(wxHORIZONTAL);
	m_sH_UpperBitmap->Add(m_bb_egsoftweb, 1, wxALIGN_CENTER, 10);
...
{
	wxLaunchDefaultBrowser(wxT("http://www.egsoftweb.in"), wxBROWSER_NOBUSYCURSOR);
}
